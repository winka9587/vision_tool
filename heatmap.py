...
def get_color_map_feature_pair(feature1, feature2):
    tsne_results_raw1 = embed_tsne_pair(feature1)
    tsne_results_raw2 = embed_tsne_pair(feature2)
    tsne_min = np.min([np.min(tsne_results_raw1), np.min(tsne_results_raw2)])
    tsne_max = np.max([np.max(tsne_results_raw1), np.max(tsne_results_raw2)])
    tsne_results1 = (tsne_results_raw1 - tsne_min) / (tsne_max - tsne_min)
    tsne_results2 = (tsne_results_raw2 - tsne_min) / (tsne_max - tsne_min)
    color1 = get_color_map(tsne_results1)
    color2 = get_color_map(tsne_results2)
