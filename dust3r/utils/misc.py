...
# 检查相邻的元素是否"成对对称互换", 期待的对称情况是instance满足[a,b,c,d]和[b,a,d,c].
def is_symmetrized(gt1, gt2):
    x = gt1['instance']
    y = gt2['instance']
    if len(x) == len(y) and len(x) == 1:  # 当instance的长度都为1时, 肯定不对称
        return False  # special case of batchsize 1
    ok = True
    for i in range(0, len(x), 2):
        ok = ok and (x[i] == y[i + 1]) and (x[i + 1] == y[i])  # 步长为2, 每次检查相邻一对instance是否对称
    return ok
